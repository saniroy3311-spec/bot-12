...
class OrderManager:
    ...
    def _open_long(self, signal_type, price, lots, timeframe):
        ...
        try:
            ...
            # FIX 6c: Brief pause so Delta India registers the position before SL placement.
            # Placing SL immediately after market fill can cause 400 errors because the
            # exchange hasn't settled the position yet and rejects reduce_only orders.
            time.sleep(0.5)

            # FIX 4: Recalculate SL/TP from ACTUAL fill price, not signal close price
            # FIX 6d: Enforce minimum stop_dist so SL is never placed dangerously tight
            MIN_STOP_DIST = 80.0  # minimum $80 BTC distance — tune if needed
            stop_dist = max(stop_dist, MIN_STOP_DIST)
            sl = round(entry_price - stop_dist, 1)
            tp = round(entry_price + stop_dist * (info.get("rr") or 4.0), 1)
            ...
            try:
                ...
                # FIX 5: SL placement failed — position is live on exchange with NO stop
                # Immediately emergency-close to avoid unprotected exposure
                logger.error(f"SL placement failed after entry fill: {sl_err} — emergency closing position")
                try:
                    self.delta.close_position()
                except Exception as close_err:
                    logger.error(f"Emergency close also failed: {close_err} — MANUAL ACTION REQUIRED")
                return {"status": "error", "msg": f"SL failed, position emergency-closed: {sl_err}"}
            ...
            try:
                ...
                # TP failed but SL is live — position is protected, log and continue
                logger.error(f"TP placement failed (SL is active, trade continues): {tp_err}")
    ...
    def _open_short(self, signal_type, price, lots, timeframe):
        ...
        try:
            ...
            try:
                ...
                # FIX 5: SL placement failed — emergency close to avoid unprotected short
                logger.error(f"SL placement failed after entry fill: {sl_err} — emergency closing position")
                try:
                    self.delta.close_position()
                except Exception as close_err:
    ...
    def _fetch_ohlcv(self, timeframe=None):
        ...
        try:
            ...
            if not candles:
                logger.error(f"Empty result from Delta India. Full response: {data}")
