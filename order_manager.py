...
class OrderManager:
    ...
    def _fetch_ohlcv(self, timeframe=None):
        ...
        try:
            # Delta India candles API (public, no auth needed)
            ...
            if not candles:
                raise ValueError(f"No candles returned from Delta India for {SYMBOL} tf={tf}")
            ...
            logger.info(f"Fetched {len(df)} candles from Delta India | tf={tf}")
            return df
